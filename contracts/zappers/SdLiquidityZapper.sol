...
contract SdLiquidityZapper {
    ...
    function zapToSdBalPool(
        ...
        if (_lockAmountPercentage > 0) {
            lockAmount = (bptReceived * _lockAmountPercentage) / 10_000;
     ...
     */
    function _provideLiquidityCurve(address _pool, uint256[2] memory _amounts, uint256 _minAmount) internal {
     ...
     */
    function _lockTokens(address _token, uint256 _amount, bool _lock) internal returns (uint256 sdReceived) {
    ...
    function _provideLiquidityBalancer(
        ...
        IBalancerVault.JoinPoolRequest memory pr =
            IBalancerVault.JoinPoolRequest(_assets, _maxAmountsIn, abi.encode(1, _maxAmountsIn, _minAmount), false);
