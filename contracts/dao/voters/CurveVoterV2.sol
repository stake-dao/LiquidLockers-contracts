...
import "../../strategies/curve/CurveStrategy.sol";
...
contract CurveVoterV2 {
    ...
    function votePct(uint256 _voteId, uint256 _yeaPct, uint256 _nayPct, address _curveVoting) external {
        require(msg.sender == governance, "!governance");
        bytes memory voteData =
            abi.encodeWithSignature("votePct(uint256,uint256,uint256,bool)", _voteId, _yeaPct, _nayPct, false);
        (bool success,) = CurveStrategy(curveStrategy).execute(
            crvLocker, 0, abi.encodeWithSignature("execute(address,uint256,bytes)", _curveVoting, 0, voteData)
        );
        require(success, "Voting failed!");
    }
    ...
    /// @param _curveVoting address of the related curve voting contract
    function vote(uint256 _voteData, bool _supports, address _curveVoting) external {
        require(msg.sender == governance, "!governance");
