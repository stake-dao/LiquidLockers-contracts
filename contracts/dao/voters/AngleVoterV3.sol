...
contract AngleVoterV3 {
    ...
    function claimRewardFromMerkle(uint256 _totalAmount, bytes32[][] calldata _proofs, Claim calldata _claim)
        external
    {
        ...
        for (uint256 i; i < claim.gauges.length;) {
            data = abi.encodeWithSignature("deposit_reward_token(address,uint256)", ANGLE, claim.amountsToNotify[i]);
            (success,) = AngleStrategy(angleStrategy).execute(
                ANGLE_LOCKER, 0, abi.encodeWithSignature("execute(address,uint256,bytes)", claim.gauges[i], 0, data)
            );
        ...
        for (uint256 i; i < claim.feeRecipients.length;) {
            data = abi.encodeWithSignature("transfer(address,uint256)", claim.feeRecipients[i], claim.feeAmounts[i]);
            (success,) = AngleStrategy(angleStrategy).execute(
                ANGLE_LOCKER, 0, abi.encodeWithSignature("execute(address,uint256,bytes)", ANGLE, 0, data)
            );
            unchecked {
                ++i;
            }
