...
contract StrategyProxy {
	...
	FeeDistribution public feeDistribution = FeeDistribution(0xA464e6DCda8AC41e03616F95f4BC98a13b8922Dc);
	...
	function setFeeDistribution(FeeDistribution _feeDistribution) external onlyGovernance {
	...
	function withdraw(
		...
		if (_snx > 0) {
			proxy.execute(snx, 0, abi.encodeWithSignature("transfer(address,uint256)", msg.sender, _snx));
		}
		return _net;
	}
	...
	function deposit(address _gauge, address _token) external {
		...
		if (_balance > 0) {
			proxy.execute(snx, 0, abi.encodeWithSignature("transfer(address,uint256)", msg.sender, _balance));
		}
	}
	...
	function ifTokenGetStuckInProxy(address asset, address recipient) external onlyGovernance returns (uint256 balance) {
	...
	// lets see
	function ifTokenGetStuck(address asset, address recipient) external onlyGovernance returns (uint256 balance) {
		require(asset != address(0), "invalid asset");
		require(recipient != address(0), "invalid recipient");
