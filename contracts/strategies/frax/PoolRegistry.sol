...
contract PoolRegistry {
    ...
    event PoolCreated(
        uint256 indexed poolid, address indexed implementation, address stakingAddress, address stakingToken
    );
    ...
    function addPool(address _implementation, address _stakingAddress, address _stakingToken) external onlyOperator {
        ...
        if (rewardImplementation != address(0)) {
            ...
            ILiquidityGaugeStratFrax(rewards).initialize(
                owner, SDT, VE_SDT, VEBOOST, distributor, poolInfo.length, address(this)
            );
        }
    ...
    function addUserVault(uint256 _pid, address _user)
        external
        onlyOperator
        returns (address vault, address stakingAddress, address stakingToken, address rewards)
    {
