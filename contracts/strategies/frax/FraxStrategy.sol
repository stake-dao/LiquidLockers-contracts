...
import "contracts/strategies/BaseStrategyV2.sol";
...
contract FraxStrategy is BaseStrategyV2 {
    ...
    function deposit(address _token, uint256 _amount, uint256 _secs) external override onlyApprovedVault {
    ...
    function withdraw(address _token, bytes32 _kek_id) external override onlyApprovedVault {
        ...
        (bool success,) =
            LOCKER.execute(gauge, 0, abi.encodePacked(FraxStakingRewardsMultiGauge.withdrawLocked.selector, _kek_id));
    ...
    function _distributeFees(address _gauge, address _rewardToken, uint256 _rewardBalance)
        internal
    ...
    function _transferFromLocker(address _token, address _recipient, uint256 _amount) internal {
        (bool success,) =
            LOCKER.execute(_token, 0, abi.encodeWithSignature("transfer(address,uint256)", _recipient, _amount));
        if (!success) {
