...
contract BalancerVault is ERC20Upgradeable {
    ...
    function provideLiquidityAndDeposit(
        ...
        IBalancerVault.JoinPoolRequest memory pr =
            IBalancerVault.JoinPoolRequest(assets, _maxAmountsIn, abi.encode(1, _maxAmountsIn, _minAmount), false);
    ...
    /// @param _amount amount to deposit
    /// @param _earn earn or not
    function _deposit(address _staker, uint256 _amount, bool _earn) internal {
        if (!_earn) {
    ...
    function withdraw(uint256 _shares) public {
        ...
        if (_shares > tokenBalance) {
            ...
            withdrawFee = (amountToWithdraw * withdrawalFee) / 10_000;
            if (withdrawFee > 0) {
