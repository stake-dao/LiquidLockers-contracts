...
import "../../accumulators/BalancerAccumulator.sol";
...
contract BalancerStrategy is BaseStrategy {
    ...
    function claim(address _token) external override {
        ...
        // Send BAL here
        (success,) =
            locker.execute(BAL, 0, abi.encodeWithSignature("transfer(address,uint256)", address(this), balMinted));
    ...
    function manageFee(MANAGEFEE _manageFee, address _gauge, uint256 _newFee) external onlyGovernanceOrFactory {
        require(_gauge != address(0), "zero address");
