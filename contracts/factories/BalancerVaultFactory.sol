...
import "../strategies/balancer/BalancerVault.sol";
...
contract BalancerVaultFactory {
    ...
    function cloneAndInit(address _balGaugeAddress) public {
        ...
        address gaugeImplAddress = _cloneAndInitGauge(vaultImplAddress, tokenSymbol);
     ...
     */
    function _cloneAndInitVault(ERC20Upgradeable _lpToken, string memory _name, string memory _symbol)
        internal
        returns (address)
    {
        BalancerVault deployed =
            cloneVault(_lpToken, keccak256(abi.encodePacked(GOVERNANCE, _name, _symbol, BALANCER_STRATEGY)));
    ...
    function _cloneAndInitGauge(address _stakingToken, string memory _symbol) internal returns (address) {
        ...
        deployed.initialize(_stakingToken, address(this), SDT, VESDT, VEBOOST, SDT_DISTRIBUTOR, _stakingToken, _symbol);
        return address(deployed);
    }
    ...
    function cloneVault(ERC20Upgradeable _lpToken, bytes32 _paramsHash) internal returns (BalancerVault) {
        address deployed =
            address(vaultImpl).cloneDeterministic(keccak256(abi.encodePacked(address(_lpToken), _paramsHash)));
    ...
    function cloneGauge(address _stakingToken, bytes32 _paramsHash) internal returns (ILiquidityGaugeStrat) {
        address deployed =
            address(GAUGE_IMPL).cloneDeterministic(keccak256(abi.encodePacked(address(_stakingToken), _paramsHash)));
