...
contract VeSDTFeeBalancerProxy is Ownable {
	...
	address[] public balToFraxPath;

	event RewardSent(uint256 claimerAmount, uint256 feeDAmount);

	constructor(address[] memory _balToFraxPath) {
		balToFraxPath = _balToFraxPath;
		IERC20(BAL).safeApprove(SUSHI_ROUTER, type(uint256).max);
	}
    ...
    /// @notice internal function to swap BAL to FRAX
    /// @dev slippageBAL = 100 for 1% max slippage
	/// @param _amount amount to swap
	function _swapOnSushi(uint256 _amount) internal returns (uint256) {
		uint256[] memory amounts = IUniswapRouter(SUSHI_ROUTER).getAmountsOut(_amount, balToFraxPath);

		uint256 minAmount = (amounts[balToFraxPath.length - 1] * (10000 - maxSlippage)) / (10000);
	...
	function claimableByKeeper() public view returns (uint256) {
		uint256 balBalance = IERC20(BAL).balanceOf(address(this));
		uint256[] memory amounts = IUniswapRouter(SUSHI_ROUTER).getAmountsOut(balBalance, balToFraxPath);
		uint256 minAmount = (amounts[balToFraxPath.length - 1] * (10000 - maxSlippage)) / (10000);
