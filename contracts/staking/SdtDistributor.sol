...
/// @title SdtDistributor
/// @author Forked from contracts developed by Curve and Frax and adapted by Sdt Core Team
...
contract SdtDistributor is SdtDistributorEvents, ReentrancyGuardUpgradeable, AccessControlUpgradeable {
    ...
    /// @notice SDT current emission rate, it is first defined in the initializer and then updated every week
    uint256 public rate;

    /// @notice Timestamp at which the current emission epoch started
    uint256 public startEpochTime;

    /// @notice Amount of SDT tokens distributed through staking at the start of the epoch
    ...
    /// @param _rewardToken Address of the SDT token
    /// @param _controller Address of the GaugeController
    /// @param _initialRate Initial SDT emission rate
    /// @param _startEpochSupply Amount of SDT tokens already distributed via liquidity mining
    ...
    /// @dev After this contract is created, the correct amount of SDDT tokens should be transferred to the contract
    ...
    function initialize(
        ...
        miningEpoch = 0;
        // Some SDT tokens should be sent to the contract directly after initialization
        rate = _initialRate;
    ...
    function _distributeReward(address gaugeAddr) internal returns (uint256 weeksElapsed, uint256 rewardTally) {
        ...
        // that need to be bridged) or is associated to an external contract of the StakeDAO Protocol
        if (gaugeType >= 2) {
            ...
            if (delegate != address(0)) {
                ...
                if (isInterfaceKnown[delegate]) {
                    ISdtMiddlemanGauge(delegate).notifyReward(gaugeAddr, rewardTally);
                }
            } else {
    ...
    /// this function has been slightly modified from Curve implementation by StakeDAO Team
    ...
    function recoverERC20(
        ...
        // If the token is the SDT token, we need to make sure that governance is not going to withdraw
    ...
    /// @notice Changes the SDT emission rate
    /// @param _newRate New SDT emission rate
    /// @dev It is important to be super wary when calling this function and to make sure that `distributeReward`
    /// has been called for all gauges in the past weeks. If not, gauges may get an incorrect distribution of SDT rewards
    ...
    function setRate(uint256 _newRate) external onlyRole(GOVERNOR_ROLE) {
        // Checking if the new rate is compatible with the amount of SDT tokens this contract has in balance
