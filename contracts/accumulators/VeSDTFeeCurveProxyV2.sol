...
contract VeSDTFeeCurveProxyV2 is Ownable {
    ...
    constructor(address[] memory _crvToUsdcPath) {
    ...
    function _swapOnSushi(uint256 _amount) internal {
        IUniswapRouter(SUSHI_ROUTER).swapExactTokensForTokens(
            _amount, 0, crvToUsdcPath, address(this), block.timestamp + 1800
        );
    }
    ...
    function _swapOnCurve(uint256 _amount) internal {
        ICurvePool(USDC_FRAX_CURVE_POOL).exchange(1, 0, _amount, 0);
    }
    ...
    function claimableByKeeper() public view returns (uint256) {
        ...
        uint256 fraxAmount = ICurvePool(USDC_FRAX_CURVE_POOL).get_dy(1, 0, amounts[crvToUsdcPath.length - 1]);
