...
contract VeSDTFeeAngleProxyV2 is Ownable {
    ...
    constructor(address[] memory _angleToAgEurPath, CurveExchangeData memory _curveExchangeData) {
    ...
    function _swapOnSushi(uint256 _amount) internal {
        IUniswapRouter(SUSHI_ROUTER).swapExactTokensForTokens(
            _amount, 0, angleToAgEurPath, address(this), block.timestamp + 1800
        );
    }
    ...
    function _swapOnCurve(uint256 _amount) internal {
        ICurveZapper(CURVE_ZAPPER).exchange(
            curveExchangeData.pool, curveExchangeData.fromIndex, curveExchangeData.toIndex, _amount, 0
        );
    }
