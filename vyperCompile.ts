...
import { VyperConfig } from "@nomiclabs/hardhat-vyper/src/types";
...
export async function compile(vyperConfig: VyperConfig, paths: ProjectPathsConfig, artifacts: Artifacts) {
  ...
  if (someContractFailed) {
    throw new NomicLabsHardhatPluginError("@nomiclabs/hardhat-vyper", "Compilation failed");
  }

  await saveLastVyperVersionUsed(vyperVersion, paths);
}

async function isAlreadyCompiled(sourceFile: string, paths: ProjectPathsConfig, vyperVersion: string) {
  ...
  const artifactPath = path.join(paths.artifacts, sourceName, `${contractName}.json`);
...
async function saveLastVyperVersionUsed(version: string, paths: ProjectPathsConfig) {
  const filePath = path.join(paths.cache, LAST_VYPER_VERSION_USED_FILENAME);
...
async function pullImageIfNecessary(docker: HardhatDocker, image: Image, cachePath: string) {
...
async function checkForImageUpdates(docker: HardhatDocker, image: Image, cachePath: string) {
...
async function getLastUpdateCheckDate(image: Image, cachePath: string): Promise<number | undefined> {
  ...
  try {
   ...
   Please check your internet connection.`,
    ...
    if (error instanceof DockerServerError) {
      throw new NomicLabsHardhatPluginError("@nomiclabs/hardhat-vyper", "Docker error", error);
    }
