...
describe("FXS Depositor", function () {
  ...
  before(async function () {
    ...
    const FraxLocker = await ethers.getContractFactory("Locker");
    ...
    locker = await FraxLocker.deploy(fxs.address, veFXS.address, FRAX_GAUGE_CONTROLLER, YIELD_DISTRIBUTOR, ACC);
    //random locker used to simulate other locks
    randomLocker1 = await FraxLocker.deploy(fxs.address, veFXS.address, FRAX_GAUGE_CONTROLLER, YIELD_DISTRIBUTOR, ACC);
    randomLocker2 = await FraxLocker.deploy(fxs.address, veFXS.address, FRAX_GAUGE_CONTROLLER, YIELD_DISTRIBUTOR, ACC);

    fxsDepositor = await FxsDepositor.deploy(fxs.address, locker.address, sdFXSToken.address);
    
    // Set FxsDepositor on lockers
    await locker.setTokenDepositor(fxsDepositor.address);
    await randomLocker1.setTokenDepositor(fxsDepositor.address);
    await randomLocker2.setTokenDepositor(fxsDepositor.address);
  ...
  describe("Lock Initial Action", function () {
    ...
    it("should check if all setters work correctly", async function () {
      ...
      await (await locker.setTokenDepositor(FXS_HOLDER)).wait();
      expect(await locker.tokenDepositor()).to.be.equal(FXS_HOLDER);
      await (await locker.setTokenDepositor(fxsDepositor.address)).wait();
  ...
  describe("Lock Final Actions", function () {
    ...
    it("Should claim rewards", async function () {
      ...
      await locker.claimTokenRewards(locker.governance());
